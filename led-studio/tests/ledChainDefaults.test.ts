...
describe('ledChainDefaults', () => {
  it('default chain has 5 outputs covering 580 logical LEDs', () => {
    expect(DEFAULT_CHAIN_PARTS).toHaveLength(5)
    expect(computeLedCountFromParts(DEFAULT_CHAIN_PARTS)).toBe(580)
  ...
  it('createPartAfterExisting appends after last range', () => {
    ...
    expect(next.ranges[0]).toEqual({ start: 580, end: 599 })
  })
  ...
  it('counts parallel fingers physically but only once logically', () => {
    ...
    expect(DEFAULT_LED_OUTPUTS.reduce((sum, output) => sum + physicalLedCountForOutput(output), 0)).toBe(740)
  })
})
