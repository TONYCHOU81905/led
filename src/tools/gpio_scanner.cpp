/**
 * GPIO 掃描測試韌體 - 輪流測試多個 GPIO 腳位
 *
 * 這個程序會依序測試多個 GPIO，每個腳位顯示 8 秒，
 * 讓您能看到哪個腳位可以讓 LED 亮起來。
 *
 * 測試序列：每個 GPIO 會顯示：
 * - 2秒紅色
 * - 2秒綠色
 ...
 * 總共 8 秒後切換到下一個 GPIO
 *
 * 燒錄：./scripts/fw.sh upload gpio-scanner
 */

#include <Arduino.h>
#include <FastLED.h>

// 測試的 GPIO 列表（根據文檔中安全可用的腳位）
const uint8_t TEST_GPIOS[] = {4, 5, 6, 7, 8, 15, 16, 17, 18, 21};
...
void testGPIO(uint8_t gpio) {
  ...
  switch(gpio) {
    ...
    case 6:  FastLED.addLeds<WS2812B, 6, GRB>(leds, TEST_COUNT); break;
    case 7:  FastLED.addLeds<WS2812B, 7, GRB>(leds, TEST_COUNT); break;
    case 8:  FastLED.addLeds<WS2812B, 8, GRB>(leds, TEST_COUNT); break;
...
void setup() {
  ...
  Serial.println();
  Serial.println("╔════════════════════════════════════════╗");
  Serial.println("║   GPIO 掃描測試 - LED 腳位診斷工具    ║");
  Serial.println("╚════════════════════════════════════════╝");
  Serial.println();
